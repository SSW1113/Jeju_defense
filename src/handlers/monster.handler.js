 ...
 * @param payload
 */
export const killHiddenMonsterHandler = async (userId, payload) => {
  // 유저의 현재 스테이지 배열을 가져오고, 최대 스테이지 ID를 찾기
  let currentStages = await stageManager.getStage(userId);
  if (!currentStages.length) {
    return { status: 'fail', message: 'No stages found for user' };
  ...
  // 스테이지 별 점수, 골드 증가 (임의로 2배)
  const goldToAdd = currentStageData.gold * 2;
  const scoreToAdd = currentStageData.score * 2;
  ...
  return { status: 'success', currentGold: currentGold, currentScore: currentScore, remainHiddenMonsters };
};